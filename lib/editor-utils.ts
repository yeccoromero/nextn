...
export const getOrientedBBox = (obj: SvgObject, objects: Record<string, SvgObject>) => {
...
export const getOverallBBox = (selectedObjects: SvgObject[], objects: Record<string, SvgObject>) => {
  ...
  if (selectedObjects.length === 1) {
    ...
    return getOrientedBBox(obj, objects);
  }
...
export const getLocalScale = (obj: SvgObject): { x: number, y: number } => {
  return { x: obj.scaleX ?? 1, y: obj.scaleY ?? 1 };
};
