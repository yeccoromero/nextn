...
function* walkObjectsDFS(rootIds: string[], objects: Record<string, SvgObject>, zStack: string[], depth = 0): Generator<{ id: string; depth: number }> {
  // If IDs are in the zStack, we use that for sorting (Descending Z = Top to Bottom UI).
  // If they are missing (e.g. grouped children), we assume they are provided in Paint Order (Bottom to Top).
  // In that case, we reverse them to get Top to Bottom UI order.

  let sortedRootIds: string[];
  const anyInZStack = rootIds.some(id => zStack.includes(id));

  if (anyInZStack) {
    sortedRootIds = [...rootIds].sort((a, b) => {
      const zA = zStack.indexOf(a);
      const zB = zStack.indexOf(b);
      return zB - zA;
    });
  } else {
    // Assume input is Bottom->Top (Paint Order), so Reverse it for Top->Bottom (UI Order)
    sortedRootIds = [...rootIds].reverse();
  }
