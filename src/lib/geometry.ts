...
import { getObjectCenter, rotatePoint, RS_apply, localToWorld, getLocalAnchor, worldToLocal, getWorldRotation, getWorldScale, getOrientedBoundingBox, getOppositeHandle, getHandlePosition } from "./editor-utils";
...
// - newScaleX/Y son ABSOLUTOS (no factores). Ej: para flip X => newScaleX = -(obj.scaleX??1)
export function scaleAroundWorldPivot(
  obj: SvgObject,
  newScaleX: number,
  newScaleY: number,
  pivotWorld: {x:number;y:number} | null | undefined,
  objects: Record<string, SvgObject>
): Partial<SvgObject> {
  if (!pivotWorld) {
    return { scaleX: newScaleX, scaleY: newScaleY };
  }
  // 1) Pivote en espacio del padre (props x/y están en coords del padre)
  ...
  // 2) Pivote en local del objeto (para no “correr” el anchor)
  const pLocal = worldToLocal(obj, pivotWorld, objects);

  // 3) Vector R•S(pLocal) con la ROTACIÓN LOCAL del objeto
  const theta = (obj.rotation || 0) * Math.PI / 180;
  const c = Math.cos(theta), s = Math.sin(theta);
  const sx = pLocal.x * newScaleX;
  const sy = pLocal.y * newScaleY;
  const RS_p_parent = { x: sx * c - sy * s, y: sx * s + sy * c };

  // 4) Nueva traslación local del objeto (coords del padre) para fijar el pivote mundial
  const newX = P_parent.x - RS_p_parent.x;
  const newY = P_parent.y - RS_p_parent.y;
  
  const updatesBase: Partial<SvgObject> = {
    x: newX,
    y: newY,
    scaleX: newScaleX,
    scaleY: newScaleY,
...
export function transformObjectByResize(params: ResizeParams): Partial<SvgObject> {
    ...
    const bbox = isGroup && groupBBox ? groupBBox : getOrientedBoundingBox(orig, objects);
    if (!bbox) return {};
